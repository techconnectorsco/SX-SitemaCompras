...
export class AssetService {
    ...
    static delete(assetId: number, marcaId: number): boolean {
        const info = db.prepare('UPDATE marca_assets SET deleted_at = ? WHERE id = ? AND marca_id = ?')
            .run(Math.floor(Date.now() / 1000), assetId, marcaId);
        return info.changes > 0;
    }

    /** Lee un asset y devuelve su contenido como base64 limpio (para enviar a Gemini) */
    static readAsBase64(asset: MarcaAsset): string | null {
        try {
            // El file_path es algo como "/uploads/marcas/1/assets/logo.png"
            // Reconstruimos la ruta física: process.cwd() + "/static" + "/uploads/..."
            const relativePath = asset.file_path.replace(/^\//, ''); // quitar '/' inicial
            const filePath = path.join(process.cwd(), 'static', relativePath);
            
            if (!fs.existsSync(filePath)) {
                console.warn(`[AssetService] Archivo no encontrado en disco: ${filePath}`);
                return null;
            }
