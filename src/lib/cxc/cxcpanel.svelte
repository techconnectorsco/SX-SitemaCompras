<script lang="ts">
	...
	const ayudaMetodo = $derived(
		metodo === 'cliente'
			? 'El estado de cuenta se envía directo a los correos del cliente registrados en el ERP.'
	...
	async function cargarClientes() {
		...
		try {
			...
			errorCarga = 'No se pudo cargar la lista de clientes. Verificá la conexión al sistema.';
			console.error(e);
		} finally {
			cargando = false;
		}
	...
	async function consultar() {
		...
		try {
			...
			if (res.status === 404) {
				auditError = `El cliente ${code} no existe en el ERP.`;
				return;
			}
			if (!res.ok) throw new Error(`HTTP ${res.status}`);
			auditData = await res.json();
		} catch (e) {
			auditError = 'No se pudo consultar el cliente. Verificá la conexión al sistema.';
			console.error(e);
		} finally {
			auditCargando = false;
		}
		...
		<div class="p-6">
			...
			{#if alcanceEfectivo === 'cliente'}
				...
				<p class="mt-3 text-xs text-muted-foreground">
					Las cuentas padre consolidan sus sucursales en un solo estado de cuenta. Un cliente sin saldo en el ERP no se envía.
				</p>
			{:else}
				...
				<div class="mt-6">
					...
					<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
						<div class="rounded-xl border border-border bg-muted/50 p-4 border-l-4" style="border-left-color: var(--brand-primary);">
							<p class="text-xs font-medium text-muted-foreground uppercase tracking-wider">Saldo Global en el ERP</p>
