<script lang="ts">
	...
	import FichasTecnicasTab from '$lib/components/app/content-creator/fichas-tecnicas-tab.svelte';
	import AccountSelector from '$lib/components/app/content-creator/account-selector.svelte';
	import {
		LayoutDashboard,
		Calendar,
		Layers,
	...
	// Cuenta Meta activa (selector multi-cuenta, persistida en localStorage por el selector)
	let selectedCuentaId = $state<number | null>(null);

	// Cambios sin guardar en la pestaña Prompts
	let promptsUnsaved = $state(false);

	function switchTab(tab: typeof activeTab) {
		if (activeTab === 'prompts' && promptsUnsaved && tab !== 'prompts') {
			if (!confirm('Tienes cambios sin guardar en los prompts. ¿Salir de todos modos?')) return;
		}
		activeTab = tab;
		if (tab === 'dashboard') void refreshDashboardStats();
	}
	...
	function formatTimestampToTimeInput(ts: number | null | undefined): string {
		if (!ts || isNaN(ts) || ts <= 0) return '12:00';
		const date = new Date(ts * 1000);
	...
	let posts = $state(
		data.publicaciones.map((p: any) => ({
			...
			time: formatTimestampToTimeInput(p.fecha_programada),
	...
	let tokenStats = $state(data.tokenStats);
	...
	async function refreshDashboardStats() {
		...
		try {
			const response = await fetch('/api/content-creator/ia/consumo/resumen');
			const payload = await response.json();
			if (!response.ok) throw new Error(payload.error || 'No se pudo actualizar el consumo de IA');
			tokenStats = payload.tokenStats;
		} catch (error) {
			console.error('[contentCreator] Error actualizando dashboard:', error);
		} finally {
...
<div
	class="grid min-h-[75vh] w-full grid-cols-1 overflow-hidden rounded-xl border bg-background shadow-sm lg:grid-cols-[260px_1fr]"
>
	...
	<aside class="flex flex-col justify-between border-r bg-muted/30 p-5 lg:p-6">
		<div class="space-y-6">
			...
			<div class="flex items-center gap-2.5 px-1.5 border-b pb-4">
				...
				<div>
					...
					<p class="text-[9px] font-semibold text-[#1A73C2] uppercase tracking-widest">SX IA</p>
				</div>
			</div>

			<!-- Botones de Navegación del Módulo -->
			<nav class="space-y-1">
				<p class="px-2 text-[9px] font-bold uppercase tracking-widest text-muted-foreground/85">
					Módulos
				</p>
		...
		<div class="space-y-2">
			<AccountSelector bind:cuentaId={selectedCuentaId} />
			<div class="rounded-lg bg-muted/60 p-3 border">
				<div class="flex items-center gap-2">
					<div
						class={`h-2 w-2 rounded-full ${selectedCuentaId ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`}
					></div>
					<span class="text-[10px] font-medium text-muted-foreground">
						{selectedCuentaId ? 'Cuenta activa seleccionada' : 'Faltan conectar cuentas'}
					</span>
				</div>
			</div>
		</div>
	...
	<div class="p-5 lg:p-6 bg-background">
		...
		{:else if activeTab === 'metrics'}
			<MetricsTab bind:posts {catalogos} bind:cuentaId={selectedCuentaId} />
		{:else if activeTab === 'meta-hub'}
			<MetaHubTab bind:posts {catalogos} bind:cuentaId={selectedCuentaId} />
		{:else if activeTab === 'prompts'}
			<PromptsMarcas {data} bind:hasUnsavedChanges={promptsUnsaved} />
		{:else if activeTab === 'assets'}
			<AssetsTab {catalogos} />
		{:else if activeTab === 'bodegas'}
			<BodegasTab />
		{:else if activeTab === 'fichas'}
			<FichasTecnicasTab {catalogos} />
		{/if}
	</div>
</div>
